...
function calcularBecas(promedio, vulnerable, fueraManta) {
    return BECAS.filter((b) => {
        if (promedio < b.promedioMin)
            return false;
        if (b.requiereVulnerabilidad && !vulnerable)
            return false;
        if (b.requiereFueraManta && !fueraManta)
            return false;
        return true;
    });
}
...
function renderizarResultado(becas) {
    if (becas.length === 0) {
        return `
...
export function renderizarVerificador(navCallback) {
  ...
  `;
    document.getElementById("btnVerificar").addEventListener("click", () => {
        ...
        const vulnerable = document.getElementById("etnia").checked ||
            document.getElementById("discapacidad").checked;
        const fueraManta = document.getElementById("fueraSi").checked;
...
function bindVerificador(navCallback) {
    document.getElementById("btnVerificar")?.addEventListener("click", () => {
        const promedioInput = document.getElementById("promedio");
        const errorEl = document.getElementById("errorPromedio");
        const promedio = parseFloat(promedioInput.value);
...
function renderizarNavbar(activo, navCallback) {
    return `
    <header>
      <nav>
