...
function renderizarPasos(actual) {
...
function renderizarPaso2(nombre, email, docs) {
...
export function renderizarPostulacion(navCallback) {
    ...
    function montar() {
        let contenido = "";
        if (paso === 0)
            contenido = renderizarPaso0();
        else if (paso === 1)
            contenido = renderizarPaso1(datos.documentos);
        else if (paso === 2)
            contenido = renderizarPaso2(datos.nombre, datos.email, datos.documentos);
        app.innerHTML = `
      ${navbar()}
    ...
    function bindPaso() {
        ...
        if (paso === 2) {
            ...
            document.getElementById("btnEnviar")?.addEventListener("click", () => {
                ...
                const solicitudes = JSON.parse(localStorage.getItem("becauleam_solicitudes") || "[]");
