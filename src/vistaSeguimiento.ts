// Módulo 3: Seguimiento de Solicitud — Jandry Sánchez
import type { Solicitud, EstadoSolicitud } from "./tipos";
import { obtenerSolicitudes, actualizarEstado } from "./storage";

const coloresEstado: Record<EstadoSolicitud, string> = {
  "Recibida":    "#3B82F6",
  "En revisión": "#F59E0B",
  "Aprobada":    "#10B981",
  "Rechazada":   "#EF4444",
...
function navbar(activo: string): string {
  return `
    <header>
      <nav>
...
function renderizarTimeline(solicitud: Solicitud): string {
  ...
  const pasosHtml = pasos.map((paso) => {
    let clase = "pendiente";
    if (paso === "Recibida") clase = "listo";
    if (paso === "En revisión" &&
      (solicitud.estado === "En revisión" || solicitud.estado === "Aprobada")) clase = "listo";
    if (paso === solicitud.estado) clase = "activo";
    return `
...
export function renderizarEncargado(navCallback: (modulo: string) => void): void {
  ...
  document.querySelectorAll<HTMLButtonElement>(".btn-actualizar").forEach((btn) => {
    btn.addEventListener("click", () => {
      const id = btn.dataset.id!;
      const select = document.querySelector<HTMLSelectElement>(`.select-estado[data-id="${id}"]`)!;
      const input = document.querySelector<HTMLInputElement>(`.input-motivo[data-id="${id}"]`)!;
...
function bindNav(navCallback: (modulo: string) => void): void {
